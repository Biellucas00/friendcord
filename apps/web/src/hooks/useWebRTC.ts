...
const isolateVoice = async (sourceStream: MediaStream): Promise<IsolatedVoice> => {
  ...
  try {
    ...
    const rnnoise = new RnnoiseWorkletNode(context, { wasmBinary, maxChannels: 1 });
    ...
    return { stream: new MediaStream([processedTrack, ...sourceStream.getVideoTracks()]), close: () => { source.disconnect(); rnnoise.disconnect(); rnnoise.destroy(); sourceTrack.stop(); processedTrack.stop(); void context.close(); } };
  } catch (error) {
    console.warn("RNNoise indisponível; mantendo supressão nativa do navegador.", error);
    void context.close();
    return { stream: sourceStream, close: () => sourceTrack.stop() };
  }
};
