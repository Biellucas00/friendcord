  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("message:send", async ({ channelId, body, attachmentId }) => {
      ...
      const result = await query<any>(`INSERT INTO messages(channel_id,author_id,body,attachment_id)
        SELECT $1,$2,$3,$4 WHERE EXISTS(SELECT 1 FROM channels c JOIN memberships m ON m.room_id=c.room_id LEFT JOIN room_role_permissions rp ON rp.room_id=m.room_id AND rp.role=m.role WHERE c.id=$1 AND m.user_id=$2 AND (m.role='owner' OR $5='superadmin' OR coalesce((rp.permissions->>'administrator')::boolean,false) OR coalesce((rp.permissions->>'sendMessages')::boolean,true) OR EXISTS(SELECT 1 FROM server_role_members srm JOIN server_roles sr ON sr.id=srm.role_id WHERE srm.user_id=$2 AND sr.room_id=c.room_id AND (coalesce((sr.permissions->>'administrator')::boolean,false) OR coalesce((sr.permissions->>'sendMessages')::boolean,false)))) AND ${categoryAccessSql("$2","$5")} AND (m.role IN ('owner','admin') OR $5='superadmin' OR (NOT EXISTS(SELECT 1 FROM channel_access ca WHERE ca.channel_id=c.id) AND NOT EXISTS(SELECT 1 FROM channel_role_access cra WHERE cra.channel_id=c.id)) OR EXISTS(SELECT 1 FROM channel_access ca WHERE ca.channel_id=c.id AND ca.user_id=$2) OR EXISTS(SELECT 1 FROM channel_role_access cra JOIN server_role_members srm ON srm.role_id=cra.role_id WHERE cra.channel_id=c.id AND srm.user_id=$2)))
    ...
    socket.on("ai:ask", async ({ channelId, receiverId, provider, prompt }) => { try {
      ...
      if (channelId) { const allowed = await query(`SELECT 1 FROM channels c JOIN memberships m ON m.room_id=c.room_id WHERE c.id=$1 AND m.user_id=$2 AND ${categoryAccessSql("$2","$3")}`, [channelId, user.id, user.siteRole ?? "user"]); if (!allowed.rowCount) return; const message = (await query<any>("INSERT INTO messages(channel_id,author_id,body) VALUES($1,$2,$3) RETURNING id,channel_id AS \"channelId\",body,created_at AS \"createdAt\"", [channelId, bot.id, answer])).rows[0]; io.to(`channel:${channelId}`).emit("message:new", { ...message, author: bot, attachment: null }); }
    ...
    socket.on("call:join", async (channelId) => { const allowed = await query(`SELECT 1 FROM channels c JOIN memberships m ON m.room_id=c.room_id LEFT JOIN room_role_permissions rp ON rp.room_id=m.room_id AND rp.role=m.role WHERE c.id=$1 AND c.kind='voice' AND m.user_id=$2 AND ${categoryAccessSql("$2","$3")} AND (m.role='owner' OR $3='superadmin' OR coalesce((rp.permissions->>'administrator')::boolean,false) OR coalesce((rp.permissions->>'connectVoice')::boolean,true) OR EXISTS(SELECT 1 FROM server_role_members srm JOIN server_roles sr ON sr.id=srm.role_id WHERE srm.user_id=$2 AND sr.room_id=c.room_id AND (coalesce((sr.permissions->>'administrator')::boolean,false) OR coalesce((sr.permissions->>'connectVoice')::boolean,false)))) AND (m.role IN ('owner','admin') OR $3='superadmin' OR (NOT EXISTS(SELECT 1 FROM channel_access ca WHERE ca.channel_id=c.id) AND NOT EXISTS(SELECT 1 FROM channel_role_access cra WHERE cra.channel_id=c.id)) OR EXISTS(SELECT 1 FROM channel_access ca WHERE ca.channel_id=c.id AND ca.user_id=$2) OR EXISTS(SELECT 1 FROM channel_role_access cra JOIN server_role_members srm ON srm.role_id=cra.role_id WHERE cra.channel_id=c.id AND srm.user_id=$2))`, [channelId, user.id, user.siteRole ?? "user"]); if (!allowed.rowCount) return; const room = `call:${channelId}`; const peers = await io.in(room).fetchSockets(); socket.join(room); for (const peer of peers) { peer.emit("webrtc:peer-joined", { socketId: socket.id, user }); if (peer.data.callState) socket.emit("call:state", { socketId: peer.id, user: peer.data.user as AuthUser, ...peer.data.callState }); } socket.data.callChannelId = channelId; socket.data.callState = { audioEnabled: true, videoEnabled: false, screenSharing: false }; socket.to(room).emit("call:state", { socketId: socket.id, user, ...socket.data.callState }); });
