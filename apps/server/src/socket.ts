...
export function attachSocket(server: Server) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("call:roster-request", async (roomId) => {
      const allowed = user.siteRole === "superadmin" || !!(await query("SELECT 1 FROM memberships WHERE room_id=$1 AND user_id=$2", [roomId, user.id])).rowCount;
      if (!allowed) return;
